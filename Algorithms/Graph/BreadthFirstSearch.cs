...
public class BreadthFirstSearch<T> : IGraphSearch<T> where T : IComparable<T>
{
    ...
    private void Bfs(IDirectedWeightedGraph<T> graph, Vertex<T> startVertex, Action<Vertex<T>>? action,
        HashSet<Vertex<T>> visited)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (currentVertex == null || visited.Contains(currentVertex)) continue;

            foreach (var vertex in graph.GetNeighbors(currentVertex)) queue.Enqueue(vertex!);
