...
public static class PrimMatrix
{
    ...
    public static float[,] Solve(float[,] adjacencyMatrix, int start)
    {
        ...
        for (var i = 0; i < numNodes - 1; i++) GetNextNode(adjacencyMatrix, key, added, parent);
    ...
    private static void ValidateMatrix(float[,] adjacencyMatrix)
    {
        ...
        for (var i = 0; i < adjacencyMatrix.GetLength(0); i++)
        {
            ...
            for (var j = 0; j < adjacencyMatrix.GetLength(0); j++)
            {
                ...
                if (!connection && float.IsFinite(adjacencyMatrix[i, j])) connection = true;
            }

            if (!connection) throw new ArgumentException("Graph must be connected!");
        }
    }
