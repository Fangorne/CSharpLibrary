...
public static class Kruskal
{
    ...
    public static float[,] Solve(float[,] adjacencyMatrix)
    {
        ...
        for (var i = 0; i < numNodes; i++) nodes[i] = set.MakeSet(i);
    ...
    public static Dictionary<int, float>[] Solve(Dictionary<int, float>[] adjacencyList)
    {
        ...
        for (var i = 0; i < numNodes; i++) mst[i] = new Dictionary<int, float>();
    ...
    {
        if (adj.GetLength(0) != adj.GetLength(1)) throw new ArgumentException("Matrix must be square!");
    ...
    private static (int, int)[] Solve(DisjointSet<int> set, Node<int>[] nodes, float[] edgeWeights,
        (int, int)[] connections)
    {
