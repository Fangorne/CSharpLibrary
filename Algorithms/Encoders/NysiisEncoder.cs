...
public class NysiisEncoder
{
    ...
    public string Encode(string text)
    {
        ...
        for (var i = 1; i < text.Length; i++) text = ReplaceStep(text, i);
    ...
    private string ReplaceStep(string text, int i)
    {
        ...
        if (text[i] == 'H')
        {
            if (!Vowels.Contains(text[i - 1])) return ReplaceWithPrevious();

            if (i < text.Length - 1 && !Vowels.Contains(text[i + 1])) return ReplaceWithPrevious();
        }

        // [vowel]W -> [vowel]
        if (text[i] == 'W' && Vowels.Contains(text[i - 1])) return ReplaceWithPrevious();
