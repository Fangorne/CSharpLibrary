...
public class FeistelCipher : IEncoder<uint>
{
    ...
    ///     Error: key should be more than 0x00001111 for better encoding, key=0 will throw
    ///     DivideByZero exception.
    /// </exception>
    /// <returns>Encoded text.</returns>
    public string Encode(string text, uint key)
    {
        var blocksListPlain = SplitTextToBlocks(text);
        StringBuilder encodedText = new();

        foreach (var block in blocksListPlain)
        {
            uint temp = 0;

            // decompose a block to two subblocks 0x0123456789ABCDEF => 0x01234567 & 0x89ABCDEF
            var rightSubblock = (uint) (block & 0x00000000FFFFFFFF);
            var leftSubblock = (uint) (block >> 32);
    ...
    /// </exception>
    /// <exception cref="ArgumentException">
    ///     Error: The length of text should be divisible by 16 as it the block lenght is 16
    ///     bytes.
    /// </exception>
    ...
    public string Decode(string text, uint key)
    {
        ...
        var blocksListEncoded = GetBlocksFromEncodedText(text);
        StringBuilder decodedTextHex = new();

        foreach (var block in blocksListEncoded)
        {
            uint temp = 0;
    ...
    private static List<ulong> SplitTextToBlocks(string text)
    {
        ...
        var textArray = Encoding.ASCII.GetBytes(text);
        var offset = 8;
        for (var i = 0; i < text.Length; i += 8)
        {
            ...
            if (i > text.Length - 8) offset = text.Length - i;

            var block = Convert.ToHexString(textArray, i, offset);
    ...
    private static List<ulong> GetBlocksFromEncodedText(string text)
    {
        ...
        {
            var block = Convert.ToUInt64(text.Substring(i, 16), 16);
    ...
    private static uint GetRoundKey(uint key, int round)
    {
        ...
        var a = (uint) Math.Pow(key, round + 2);
        return a ^ key;
    }
