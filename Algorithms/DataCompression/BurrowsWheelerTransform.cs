...
public class BurrowsWheelerTransform
{
    ...
    {
        if (s.Length == 0) return (string.Empty, 0);
    ...
    public string Decode(string s, int index)
    {
        ...
        {
            for (var j = 0; j < s.Length; j++) rotations[j] = s[j] + rotations[j];
    ...
    private string[] GetRotations(string s)
    {
        ...
        for (var i = 0; i < s.Length; i++) result[i] = s.Substring(i) + s.Substring(0, i);
