...
public class ShannonFanoCompressor
{
    ...
    {
        if (string.IsNullOrEmpty(uncompressedText)) return (string.Empty, new Dictionary<string, string>());
    ...
    {
        if (node.Data.Length == 1) return node;
    ...
    private ListNode GetListNodeFromText(string text)
    {
        ...
        {
            var ch = text[i];
            if (!occurenceCounts.ContainsKey(ch)) occurenceCounts.Add(ch, 0);
    ...
    public class ListNode
    {
        ...
        public ListNode((char symbol, double frequency)[] data)
        {
            Data = data;
        }
