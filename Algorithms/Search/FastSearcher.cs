...
public class FastSearcher
{
    ...
    {
        if (array.Length == 0) throw new ItemNotFoundException();

        if (item < array[0] || item > array[^1]) throw new ItemNotFoundException();

        if (array[0] == array[^1]) return item == array[0] ? 0 : throw new ItemNotFoundException();
    ...
    {
        if (item < array[left]) return (0, left - 1);

        if (item < array[right]) return (left, right - 1);
