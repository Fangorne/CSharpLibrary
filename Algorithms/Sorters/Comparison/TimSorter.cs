...
/// <summary>
///     Timsort is a hybrid stable sorting algorithm, derived from merge sort and insertion sort, designed to perform well
...
public class TimSorter<T> : IComparisonSorter<T>
{
    ...
    private static int BoundLeftShift(int shiftable)
    {
        return shiftable << 1 < 0
            ? (shiftable << 1) + 1
            : int.MaxValue;
    }
    ...
    {
        var runHi = start + 1;
        if (runHi == array.Length) return 1;

        // Find end of run, and reverse range if descending
        if (comparer.Compare(array[runHi++], array[start]) < 0)
        {
            // Descending
            while (runHi < array.Length && comparer.Compare(array[runHi], array[runHi - 1]) < 0) runHi++;
    ...
    /// <param name="first">
    ///     The index of the first element in the range that is not already known to be sorted, must be between
    ///     start and end.
    /// </param>
    private void BinarySort(T[] array, int start, int end, int first)
    {
        if (first >= end || first <= start) first = start + 1;
    ...
    private void MergeCollapse(T[] array)
    {
        while (stackSize > 1)
        {
            ...
            {
                if (runLengths[n - 1] < runLengths[n + 1]) n--;
    ...
    private void MergeForceCollapse(T[] array)
    {
        ...
        {
            var n = stackSize - 2;
            if (n > 0 && runLengths[n - 1] < runLengths[n + 1]) n--;
    ...
    private void Merge(T[] array, int baseA, int lenA, int baseB, int lenB)
    {
        ...
        if (!NeedsMerge(left, right, ref dest))
    ...
    private bool StableMerge(TimChunk<T> left, TimChunk<T> right, ref int dest, int gallop)
    {
        do
        {
            if (comparer.Compare(right.Array[right.Index], left.Array[left.Index]) < 0)
            {
                ...
                right.Wins++;
                left.Wins = 0;
                if (--right.Remaining == 0) return true;
            }
            else
            {
                ...
                left.Wins++;
                right.Wins = 0;
                if (--left.Remaining == 1) return true;
            }
