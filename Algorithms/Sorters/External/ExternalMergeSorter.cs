...
public class ExternalMergeSorter<T> : IExternalSorter<T>
{
    ...
    public void Sort(ISequentialStorage<T> mainMemory, ISequentialStorage<T> temporaryMemory,
        IComparer<T> comparer)
    {
        ...
        for (var stripLength = 1L; stripLength < totalLength; stripLength *= 2)
        {
            ...
            for (var i = 0L; i < stripLength; i++) right.Read();
            ...
            for (var i = 0L; i < totalLength + stripLength - rightStripStart; i++) output.Write(right.Read());

            (source, temp) = (temp, source);
        }

        if (source == originalSource) return;

        using var sorted = source.GetReader();
        using var dest = originalSource.GetWriter();
        for (var i = 0; i < totalLength; i++) dest.Write(sorted.Read());
    }
    ...
    private static void Merge(ISequentialStorageReader<T> left, ISequentialStorageReader<T> right,
        ISequentialStorageWriter<T> output, long leftLength, long rightLength, IComparer<T> comparer)
    {
        var leftIndex = 0L;
        var rightIndex = 0L;
        ...
        while (true)
            ...
            else
            {
                output.Write(r);
                rightIndex++;
                if (rightIndex == rightLength) break;
    ...
    {
        for (var i = 0; i < count; i++) to.Write(from.Read());
    }
