...
public class GaussOptimization
{
    ...
    /// <param name="eps">
    ///     This value is used to control the accuracy of the optimization. In case if the error is less than eps,
        ...
        double x2)
    {
        ...
        while (Math.Abs(error) > eps)
        {
            ...
            var bottom = func(x1, x2 - step);
            var top = func(x1, x2 + step);
            var left = func(x1 - step, x2);
            var right = func(x1 + step, x2);

            // Determination of the best option.
            var possibleFunctionValues = new List<double> {bottom, top, left, right};
            var maxValue = possibleFunctionValues.Max();
