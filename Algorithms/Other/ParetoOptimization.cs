...
public class ParetoOptimization
{
    ...
    public List<List<decimal>> Optimize(List<List<decimal>> matrix)
    {
        ...
        {
            for (var j = i + 1; j < optimizedMatrix.Count; j++)
            {
                var directParwiseDifference = GetMinimalPairwiseDifference(optimizedMatrix[i], optimizedMatrix[j]);
                var indirectParwiseDifference = GetMinimalPairwiseDifference(optimizedMatrix[j], optimizedMatrix[i]);
                /*
