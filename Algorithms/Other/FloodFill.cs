...
public static class FloodFill
{
    ...
    private static readonly List<(int xOffset, int yOffset)> Neighbors = new()
    ...
    public static void BreadthFirstSearch(Bitmap bitmap, (int x, int y) location, Color targetColor,
        Color replacementColor)
    {
        ...
        while (queue.Count > 0) BreadthFirstFill(bitmap, location, targetColor, replacementColor, queue);
    }
    ...
    /// <param name="targetColor">The old color to be replaced.</param>
    /// <param name="replacementColor">The new color to replace the old one.</param>
    public static void DepthFirstSearch(Bitmap bitmap, (int x, int y) location, Color targetColor,
        Color replacementColor)
    {
        if (location.x < 0 || location.x >= bitmap.Width || location.y < 0 || location.y >= bitmap.Height)
            throw new ArgumentOutOfRangeException(nameof(location),
    ...
    private static void BreadthFirstFill(Bitmap bitmap, (int x, int y) location, Color targetColor,
        Color replacementColor, List<(int x, int y)> queue)
    {
        var currentLocation = queue[0];
        queue.RemoveAt(0);
        ...
        if (bitmap.GetPixel(currentLocation.x, currentLocation.y) == targetColor)
        {
            ...
            {
                var x = currentLocation.x + Neighbors[i].xOffset;
                var y = currentLocation.y + Neighbors[i].yOffset;
                if (x >= 0 && x < bitmap.Width && y >= 0 && y < bitmap.Height) queue.Add((x, y));
            }
        }
    }

    private static void DepthFirstFill(Bitmap bitmap, (int x, int y) location, Color targetColor,
        Color replacementColor)
    {
        if (bitmap.GetPixel(location.x, location.y) == targetColor)
        {
            ...
            for (var i = 0; i < Neighbors.Count; i++)
            {
                var x = location.x + Neighbors[i].xOffset;
                var y = location.y + Neighbors[i].yOffset;
