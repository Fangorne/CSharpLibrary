...
public class OnesCountingSequence : ISequence
{
    ...
    public IEnumerable<BigInteger> Sequence
    {
        get
        {
            ...
            while (true)
            {
                foreach (var count in GenerateFractalCount(BigInteger.One, depth)) yield return count;
    ...
    private static IEnumerable<BigInteger> GenerateFractalCount(BigInteger i, int depth)
    {
        ...
        foreach (var firstHalf in GenerateFractalCount(i, depth - 1)) yield return firstHalf;

        foreach (var secondHalf in GenerateFractalCount(i + 1, depth - 1)) yield return secondHalf;
    }
