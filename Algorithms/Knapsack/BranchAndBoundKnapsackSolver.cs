...
public class BranchAndBoundKnapsackSolver<T>
{
    ...
    public T[] Solve(T[] items, int capacity, Func<T, int> weightSelector, Func<T, double> valueSelector)
    {
        ...
        while (nodesQueue.Count != 0)
        {
            ...
            var parent = nodesQueue.Dequeue();

            // IF it is the last level, branching cannot be performed
            if (parent.Level == items.Length - 1) continue;
            ...
            if (left.UpperBound > maxCumulativeValue && left.CumulativeWeight < capacity) nodesQueue.Enqueue(left);

            // Cumulative weight is the same as for parent node and < capacity
            if (right.UpperBound > maxCumulativeValue) nodesQueue.Enqueue(right);
        }
    ...
    /// </returns>
    private static double ComputeUpperBound(BranchAndBoundNode aNode, T[] items, int capacity,
        Func<T, int> weightSelector, Func<T, double> valueSelector)
    {
