...
public static class GaleShapley
{
    ...
    {
        if (proposers.Length != accepters.Length) throw new ArgumentException("Collections must have equal count");

        while (proposers.Any(m => !IsEngaged(m))) DoSingleMatchingRound(proposers.Where(m => !IsEngaged(m)));
    }

    private static bool IsEngaged(Proposer proposer)
    {
        return proposer.EngagedTo is not null;
    }
