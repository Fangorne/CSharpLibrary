...
public static class AutomorphicNumber
{
    ...
    /// <summary>
    ///     Generates a list of automorphic numbers that are between <paramref name="lowerBound" /> and
    ...
    /// <returns>
    ///     A list that contains all of the automorphic numbers between <paramref name="lowerBound" /> and
    ///     <paramref name="upperBound" /> inclusive.
    /// </returns>
    /// <exception cref="ArgumentException">
    ///     If the <paramref name="lowerBound" />
    ///     or <paramref name="upperBound" /> is not greater than zero
    ///     or <paramref name="upperBound" />is lower than the <paramref name="lowerBound" />.
    /// </exception>
    public static IEnumerable<int> GetAutomorphicNumbers(int lowerBound, int upperBound)
    {
        if (lowerBound < 1) throw new ArgumentException("Lower bound must be greater than 0.");

        if (upperBound < 1) throw new ArgumentException("Upper bound must be greater than 0.");
    ...
    {
        if (number < 1) throw new ArgumentException("An automorphic number must always be positive.");

        var square = BigInteger.Pow(number, 2);
