...
/// <summary>
///     In number theory, a Keith number or repfigit number is a natural number n in a given number base b with k digits
///     such that
///     when a sequence is created such that the first k terms are the k digits of n and each subsequent term is the sum of
///     the
...
public static class KeithNumberChecker
{
    ...
    {
        if (number < 0) throw new ArgumentException($"{nameof(number)} cannot be negative");
        ...
        while (sum < number)
        {
            ...
            for (var j = 1; j <= digitsInNumber; j++) sum += termsArray[k - j];
