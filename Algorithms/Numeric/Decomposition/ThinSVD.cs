...
public static class ThinSvd
{
    ...
    public static double[] RandomUnitVector(int dimensions)
    {
        ...
        for (var i = 0; i < dimensions; i++) result[i] = 2 * random.NextDouble() - 1;
    ...
    public static double[] Decompose1D(double[,] matrix, double epsilon, int maxIterations)
    {
        ...
        var currIteration = RandomUnitVector(n);
        var b = matrix.Transpose().Multiply(matrix);
        do
        {
            ...
            if (mag > epsilon) currIteration = currIteration.Scale(1 / mag);
        ...
        int maxIterations)
    {
        ...
        var sigmas = new double[numValues];
        var us = new double[m, numValues];
        var vs = new double[n, numValues];

        // keep track of progress
        var remaining = matrix.Copy();

        // for each singular value
        for (var i = 0; i < numValues; i++)
        {
            // compute the v singular vector
            var v = Decompose1D(remaining, epsilon, maxIterations);
            var u = matrix.MultiplyVector(v);

            // compute the contribution of this pair of singular vectors
            var contrib = u.OuterProduct(v);
            ...
            for (var j = 0; j < u.Length; j++) us[j, i] = u[j];

            for (var j = 0; j < v.Length; j++) vs[j, i] = v[j];
