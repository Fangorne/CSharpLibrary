...
public static class Maclaurin
{
    ...
    {
        return Enumerable.Range(0, n).Sum(i => ExpTerm(x, i));
    }
    ...
    {
        return Enumerable.Range(0, n).Sum(i => SinTerm(x, i));
    }
    ...
    {
        return Enumerable.Range(0, n).Sum(i => CosTerm(x, i));
    }
    ...
    {
        return ErrorTermWrapper(x, error, ExpTerm);
    }
    ...
    {
        return ErrorTermWrapper(x, error, SinTerm);
    }
    ...
    {
        return ErrorTermWrapper(x, error, CosTerm);
    }
    ...
    {
        if (error <= 0.0 || error >= 1.0) throw new ArgumentException("Error value is not on interval (0.0; 1.0).");
    ...
    private static double ExpTerm(double x, int i)
    {
        return Math.Pow(x, i) / Factorial.Calculate(i);
    }
    ...
    {
        return Math.Pow(-1, i) / Factorial.Calculate(2 * i + 1) * Math.Pow(x, 2 * i + 1);
    }
    ...
    {
        return Math.Pow(-1, i) / Factorial.Calculate(2 * i) * Math.Pow(x, 2 * i);
    }
