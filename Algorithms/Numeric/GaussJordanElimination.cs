...
public class GaussJordanElimination
{
    ...
    public bool Solve(double[,] matrix)
    {
        ...
        if (!CanMatrixBeUsed(matrix)) throw new ArgumentException("Please use a n*(n+1) matrix with Length > 0.");
    ...
    {
        return matrix?.Length == RowCount * (RowCount + 1) && RowCount > 1;
    }
    ...
    private bool ElementaryReduction(ref double[,] matrix)
    {
        for (var row = RowCount - 1; row >= 0; row--)
        {
            ...
            for (var i = 0; i < RowCount + 1; i++) matrix[row, i] /= element;
