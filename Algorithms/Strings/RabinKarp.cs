...
public static class RabinKarp
{
    ...
    public static List<int> FindAllOccurrences(string text, string pattern)
    {
        ...
        // p_pow[i] = P^i mod M
        var pPow = new ulong[Math.Max(pattern.Length, text.Length)];
        pPow[0] = 1;
        for (var i = 1; i < pPow.Length; i++) pPow[i] = pPow[i - 1] * p % m;
        ...
        var hashT = new ulong[text.Length + 1];
        for (var i = 0; i < text.Length; i++) hashT[i + 1] = (hashT[i] + text[i] * pPow[i]) % m;

        // hash_s is equal to sum of the hash values of the pattern (mod M).
        ulong hashS = 0;
        for (var i = 0; i < pattern.Length; i++) hashS = (hashS + pattern[i] * pPow[i]) % m;
