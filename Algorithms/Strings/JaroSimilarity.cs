...
///         Result is between 0 and 1 where 0 represnts that there is no similarity between strings and 1 represents equal
///         strings.
...
public static class JaroSimilarity
{
    ...
    private static int CalculateTranspositions(string s1, string s2, bool[] s1MatchedIndeces, bool[] s2MatchedIndeces)
    {
        ...
        for (var s1Index = 0; s1Index < s1.Length; s1Index++)
            ...
            {
                while (!s2MatchedIndeces[s2Index]) s2Index++;

                if (s1[s1Index] != s2[s2Index]) transpositions++;
