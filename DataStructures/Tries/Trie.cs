...
/// <summary>
///     A Trie is a data structure (particular case of m-ary tree) used to efficiently represent strings with common
///     prefixes.
///     Originally posed by E. Fredkin in 1960.
///     Fredkin, Edward (Sept. 1960), "Trie Memory", Communications of the ACM 3 (9): 490-499.
///     Its name is due to retrieval because its main application is in the field of "Information Retrieval" (information
///     retrieval).
/// </summary>
public class Trie
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="Trie" /> class. This instances was created without text strings,
    ///     generating the root node of the trie, without children.
    /// </summary>
    public Trie()
    {
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="Trie" /> class. Given a set of text strings, each of those strings
    ///     inserts them into the trie using the Insert (string) method.
    /// </summary>
    /// <param name="words">The array with text strings to insert in the trie.</param>
    public Trie(IEnumerable<string> words)
        : this()
    {
        foreach (var s in words) Insert(s);
    }
    ...
    /// <summary>
    ///     Insert a string s to the trie. The $ mark is added to the end of the chain and then it is added, this in order to
    ///     indicate the end of the chain in the trie.
    /// </summary>
    ...
    public void Insert(string s)
    {
        ...
        var index = 0;
        var match = PrefixQuery(s, ref index);
    ...
    /// <summary>
    ///     This method analyzes which is the longest common prefix of a string s in the trie. If the string is in the trie
    ///     then it is equivalent to doing Find (s).
    /// </summary>
    ...
    {
        var current = root;
        for (var i = 0; i < s.Length && current != null; i++)
            if (current[s[i]] != null)
            {
