...
public class FibonacciHeap<T> where T : IComparable
{
    ...
    public FHeapNode<T> Push(T x)
    {
        ...
        else
        {
            ...
            if (newItem.Key.CompareTo(MinItem.Key) < 0) MinItem = newItem;
        }
    ...
    public void Union(FibonacciHeap<T> other)
    {
        ...
        if (other.MinItem.Key.CompareTo(MinItem.Key) < 0) MinItem = other.MinItem;
    ...
    {
        FHeapNode<T>? z = null;
        if (MinItem == null) throw new InvalidOperationException("Heap is empty!");
        ...
        if (z.Child != null)
        {
            foreach (var x in SiblingIterator(z.Child)) x.Parent = null;
    ...
    public T Peek()
    {
        if (MinItem == null) throw new InvalidOperationException("The heap is empty");
    ...
    {
        if (MinItem == null) throw new ArgumentException($"{nameof(x)} is not from the heap");

        if (x.Key == null) throw new ArgumentException("x has no value");

        if (k.CompareTo(x.Key) > 0) throw new InvalidOperationException("Value cannot be increased");
        ...
        if (x.Key.CompareTo(MinItem.Key) < 0) MinItem = x;
    }
    ...
    {
        if (MinItem == null) throw new InvalidOperationException("Heap malformed");
