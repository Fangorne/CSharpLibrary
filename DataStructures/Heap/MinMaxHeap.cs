...
public class MinMaxHeap<T>
{
    ...
    public MinMaxHeap(IEnumerable<T>? collection = null, IComparer<T>? comparer = null)
    {
        ...
        for (var i = Count / 2 - 1; i >= 0; --i) PushDown(i);
    }
    ...
    private int IndexOfMaxChildOrGrandchild(int index)
    {
        ...
        {
            if (descendant >= Count) break;

            if (Comparer.Compare(heap[descendant], heap[resIndex]) > 0) resIndex = descendant;
        }
    ...
    {
        return grandchild > 2 && Grandparent(grandchild) == node;
    }
    ...
    private void PushDownMax(int index)
    {
        ...
        if (IsGrandchild(index, maxIndex))
        {
            if (Comparer.Compare(heap[maxIndex], heap[index]) > 0)
            {
                ...
                if (Comparer.Compare(heap[maxIndex], heap[Parent(maxIndex)]) < 0) SwapNodes(maxIndex, Parent(maxIndex));
        ...
        {
            if (Comparer.Compare(heap[maxIndex], heap[index]) > 0) SwapNodes(maxIndex, index);
        }
    }
    ...
    private void PushDownMin(int index)
    {
        ...
        if (IsGrandchild(index, minIndex))
        {
            if (Comparer.Compare(heap[minIndex], heap[index]) < 0)
            {
                ...
                if (Comparer.Compare(heap[minIndex], heap[Parent(minIndex)]) > 0) SwapNodes(minIndex, Parent(minIndex));
        ...
        {
            if (Comparer.Compare(heap[minIndex], heap[index]) < 0) SwapNodes(minIndex, index);
        }
    }
