...
public class AaTree<TKey>
{
    ...
    {
        if (!Contains(key, Root)) throw new InvalidOperationException($"{nameof(key)} is not in the tree");
    ...
    {
        if (node is null) return new AaTreeNode<TKey>(key, 1);
    ...
    private AaTreeNode<TKey>? Remove(TKey key, AaTreeNode<TKey>? node)
    {
        ...
        if (node.Right is not null) node.Right.Right = Skew(node.Right.Right);
    ...
    {
        if (node?.Left is null || node.Left.Level != node.Level) return node;
    ...
    {
        if (node?.Right?.Right is null || node.Level != node.Right.Right.Level) return node;
    ...
    private AaTreeNode<TKey> DecreaseLevel(AaTreeNode<TKey> node)
    {
        ...
        if (newLevel >= node.Level) return node;
        ...
        static int GetLevel(AaTreeNode<TKey>? x)
        {
            return x?.Level ?? 0;
        }
    }
