...
public static class Extensions
{
    ...
    {
        if (root.Left != null) FlattenTree(root.Left, list);

        list.Add(root);

        if (root.Right != null) FlattenTree(root.Right, list);
    }
    ...
    {
        if (start > end) throw new ArgumentException("The parameter's value is invalid.", nameof(start));
