...
public class ScapegoatTree<TKey> where TKey : IComparable
{
    ...
    {
        if (path.Count == 0) throw new ArgumentException("The path collection should not be empty.", nameof(path));

        var depth = 1;

        while (path.TryPop(out var next))
        {
            if (depth > next.GetAlphaHeight(Alpha)) return path.TryPop(out var parent) ? (parent, next) : (null, next);
