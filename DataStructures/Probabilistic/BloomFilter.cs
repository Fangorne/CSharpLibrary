...
public class BloomFilter<T> where T : notnull
{
    ...
    {
        numHashes = (int) Math.Ceiling(.693 * 8 * expectedNumElements /
                                       expectedNumElements); // compute optimal number of hashes
    ...
    private IEnumerable<int> GetSlots(T item)
    {
        ...
        for (var i = 0; i < numHashes; i++) yield return Math.Abs((i + 1) * hash) % sizeBits;
    }
