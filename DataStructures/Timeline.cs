...
public class Timeline<TValue> : ICollection<(DateTime Time, TValue Value)>, IEquatable<Timeline<TValue>>
{
    ...
    public TValue[] this[DateTime time]
    {
        ...
        set
        {
            ...
            foreach (var @event in overridenEvents) timeline.Remove(@event);

            foreach (var v in value) Add(time, v);
        }
    }
    ...
    public static bool operator ==(Timeline<TValue> left, Timeline<TValue> right)
    {
        ...
        if (left.Count != rightArray.Length) return false;
    ...
    {
        return timeline.Select(t => t.Time)
            .Distinct()
            .ToArray();
    }
    ...
    {
        return timeline.Where(pair => pair.Value!.Equals(value))
    ...
    {
        return timeline.Select(pair => pair.Value)
            .ToArray();
    }
    ...
    {
        return timeline.Where(pair => pair.Time == time)
    ...
    {
        return new(this.Where(pair => pair.Time < time).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Millisecond == millisecond).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Second == second).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Minute == minute).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Hour == hour).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Day == day).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.TimeOfDay == timeOfDay).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.DayOfWeek == dayOfWeek).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.DayOfYear == dayOfYear).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Month == month).ToArray());
    }
    ...
    {
        return new(timeline.Where(pair => pair.Time.Year == year).ToArray());
    }
    ...
    public void AddNow(params TValue[] value)
    {
        ...
        foreach (var v in value) Add(now, v);
    }
    ...
    {
        return timeline.Any(@event => Contains(@event.Item1, @event.Item2));
    }
    ...
    {
        var result = false;
        foreach (var (time, value) in timeline) result |= this.timeline.Remove((time, value));
    ...
    public bool RemoveTimes(params DateTime[] times)
    {
        ...
        if (!isTimeContainedInTheTimeline) return false;

        var eventsToRemove = times.SelectMany(time =>
                timeline.Where(@event => @event.Time == time))
            .ToList();

        foreach (var @event in eventsToRemove) timeline.Remove(@event);
    ...
    public bool RemoveValues(params TValue[] values)
    {
        ...
        if (!isValueContainedInTheTimeline) return false;
    ...
    {
        return timeline.ToDictionary(@event => @event.Time, @event => @event.Value);
    }

    /// <inheritdoc />
    public override bool Equals(object? obj)
    {
        return obj is Timeline<TValue> otherTimeline
               && this == otherTimeline;
    }
