...
public class AvlTree<TKey>
{
    ...
    public TKey GetMin()
    {
        if (root is null) throw new InvalidOperationException("AVL tree is empty.");
    ...
    private static AvlTreeNode<TKey> Rebalance(AvlTreeNode<TKey> node)
    {
        ...
        {
            if (node.Right!.BalanceFactor == -1) node.Right = RotateRight(node.Right);
        ...
        if (node.BalanceFactor < -1)
        {
            if (node.Left!.BalanceFactor == 1) node.Left = RotateLeft(node.Left);
