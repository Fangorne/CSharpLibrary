...
public class DirectedWeightedGraph<T> : IDirectedWeightedGraph<T>
{
    ...
    {
        if (AreAdjacent(startVertex, endVertex)) return adjacencyMatrix[startVertex.Index, endVertex.Index];
    ...
    {
        if (weight.Equals(0.0d)) throw new InvalidOperationException("Edge weight cannot be zero.");
    }
    ...
    {
        if (Count == capacity) throw new InvalidOperationException("Graph overflow.");
    }

    private void ThrowIfVertexNotInGraph(Vertex<T> vertex)
    {
        if (vertex.Graph != this) throw new InvalidOperationException($"Vertex does not belong to graph: {vertex}.");
    }
