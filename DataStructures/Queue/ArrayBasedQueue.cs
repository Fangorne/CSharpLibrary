...
public class ArrayBasedQueue<T>
{
    ...
    public T Dequeue()
    {
        if (IsEmpty()) throw new InvalidOperationException("There are no items in the queue.");

        var dequeueIndex = endIndex;
        endIndex++;
        if (endIndex >= queue.Length) endIndex = 0;
    ...
    {
        if (IsFull()) throw new InvalidOperationException("The queue has reached its capacity.");
