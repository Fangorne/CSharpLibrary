...
public class RedBlackTree<TKey>
{
        ...
        do
        {
            ...
            switch (addCase)
            {
                ...
                case 2:
                    ...
                    if (node is not null) childDir = comparer.Compare(oldParent!.Key, oldParent.Parent!.Key);
    ...
    private RedBlackTreeNode<TKey>? AddCase2(RedBlackTreeNode<TKey> node)
    {
        ...
        // Keep root black
        if (node.Parent.Parent is null) node.Parent.Color = NodeColor.Black;
    ...
    private void AddCase56(RedBlackTreeNode<TKey> node, int parentDir, int childDir)
    {
        ...
        {
            // Case 5
            if (childDir > 0) node = RotateLeft(node);
        ...
        {
            // Case 5
            if (childDir < 0) node = RotateRight(node);
    ...
    {
        if (node.Color == NodeColor.Black) return 1;

        if (node.Parent is null) return 4;
        ...
        // Case 5 & 6
        if (uncle is null || uncle.Color == NodeColor.Black) return 56;
    ...
    {
        if (node is null) throw new InvalidOperationException("Tree is empty!");

        if (!Contains(key)) throw new KeyNotFoundException($"Key {key} is not in the tree!");
    ...
    private int GetRemoveCase(RedBlackTreeNode<TKey> node)
    {
        ...
        if (distantNephew is not null && distantNephew.Color == NodeColor.Red)
            return 6;
        if (closeNewphew is not null && closeNewphew.Color == NodeColor.Red)
            return 5;
    ...
    private RedBlackTreeNode<TKey> RotateLeft(RedBlackTreeNode<TKey> node)
    {
        ...
        if (temp2 is not null) node.Left.Right!.Parent = temp1;

        if (node.Parent is null) root = node;
    ...
    private RedBlackTreeNode<TKey> RotateRight(RedBlackTreeNode<TKey> node)
    {
        ...
        if (temp2 is not null) node.Right.Left!.Parent = temp1;
