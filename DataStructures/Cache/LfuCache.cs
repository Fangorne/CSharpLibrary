...
public class LfuCache<TKey, TValue> where TKey : notnull
{
    ...
    public void Put(TKey key, TValue value)
    {
        if (cache.ContainsKey(key))
        {
            ...
            UpdateFrequency(existingNode, false);
            return;
        }

        if (cache.Count >= capacity) EvictOneItem();

        var item = new CachedItem {Key = key, Value = value};
        var newNode = new LinkedListNode<CachedItem>(item);
        UpdateFrequency(newNode, true);
    ...
    private void UpdateFrequency(LinkedListNode<CachedItem> node, bool isNew)
    {
        ...
        else
        {
            ...
            lruList.Remove(node);
            if (lruList.Count == 0 && minFrequency == item.Frequency) minFrequency++;

            item.Frequency++;
        }

        // Insert item to the end of the LRU list that corresponds to its new frequency.
        if (!frequencies.ContainsKey(item.Frequency)) frequencies[item.Frequency] = new LinkedList<CachedItem>();
