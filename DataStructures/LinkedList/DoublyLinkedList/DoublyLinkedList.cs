...
public class DoublyLinkedList<T>
{
    ...
    {
        if (Head is null) return AddHead(data);
    ...
    {
        if (existingNode == Tail) return Add(data);
        ...
        if (existingNode.Next is not null) existingNode.Next.Previous = node;
    ...
    public void Reverse()
    {
        ...
        if (temp is not null) Head = temp.Previous;
    }
    ...
    {
        if (position < 0 || position >= Count) throw new ArgumentOutOfRangeException($"Max count is {Count}");

        var current = Head;
        for (var i = 0; i < position; i++) current = current!.Next;
    ...
    public void Remove()
    {
        if (Tail is null) throw new InvalidOperationException("Cannot prune empty list");
    ...
    public int IndexOf(T data)
    {
        ...
        while (current is not null)
        {
            if ((current.Data is null && data is null) ||
                (current.Data is not null && current.Data.Equals(data))) return index;
