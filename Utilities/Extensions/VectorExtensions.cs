...
public static class VectorExtensions
{
    ...
    public static double[] Copy(this double[] vector)
    {
        ...
        for (var i = 0; i < vector.Length; i++) result[i] = vector[i];
    ...
    {
        if (lhs.Length != rhs.Length) throw new ArgumentException("Dot product arguments must have same dimension");

        double result = 0;
        for (var i = 0; i < lhs.Length; i++) result += lhs[i] * rhs[i];
    ...
    {
        var result = new double[vector.Length];
        for (var i = 0; i < vector.Length; i++) result[i] = vector[i] * factor;
    ...
    public static double[,] ToColumnVector(this double[] source)
    {
        ...
        for (var i = 0; i < source.Length; i++) columnVector[i, 0] = source[i];
    ...
    public static double[] ToRowVector(this double[,] source)
    {
        ...
        for (var i = 0; i < rowVector.Length; i++) rowVector[i] = source[i, 0];
    ...
    public static double[,] ToDiagonalMatrix(this double[] vector)
    {
        ...
        for (var i = 0; i < len; i++) result[i, i] = vector[i];
